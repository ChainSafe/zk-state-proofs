...
mod tests {
    ...
    use trie_utils::{
        constants::{DEFAULT_BLOCK_HASH, USDT_CONTRACT_ADDRESS},
        get_ethereum_account_proof_inputs, get_ethereum_transaction_proof_inputs,
    };
    ...
    #[tokio::test]
    async fn test_generate_transaction_zk_proof_sp1() {
        let start_time = Instant::now();
    ...
    #[tokio::test]
    async fn test_generate_account_zk_proof_sp1() {
        let start_time = Instant::now();
        let client = ProverClient::new();
        let mut stdin = SP1Stdin::new();
        let proof_input = serde_json::to_vec(
